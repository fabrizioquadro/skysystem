...
<form id='formulario' action="{{ route('export.imprimir') }}" method="post" target='_blank'>
    ...
    <input type="hidden" name="nm_pagina" value="Relatório de Veículos">
</form>
<div id='div_dados'>
    <div class="card mt-3">
        <div class="card-body">
            <h5 class="card-title">Relatórios Veículos: {{ "Data Geração: ".date('d/m/Y H:i:s') }}</h5>
            <div class="card mt-3">
                <div class="card-body">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            @endphp
                            @foreach($array_veiculos as $veiculo)
                                @php
                                $total_valores += $veiculo['vl_ferramentas'];
                                $total_instalacoes_totais += $veiculo['nr_intalacoes_totais'];
                                $total_instalacoes_ativas += $veiculo['nr_intalacoes_ativas'];
                                $total_instalacoes_inativas += $veiculo['nr_intalacoes_desinstaladas'];
                                @endphp
                                <tr>
                                    <td>{{ $veiculo['nm_cliente'] }}</td>
                                    <td>{{ $veiculo['nm_veiculo'] }}</td>
                                    <td class='text-center'>{{ $veiculo['nr_intalacoes_totais'] }}</td>
                                    <td class='text-center'>{{ $veiculo['nr_intalacoes_ativas'] }}</td>
                                    <td class='text-center'>{{ $veiculo['nr_intalacoes_desinstaladas'] }}</td>
                                    <td class='text-center'>{{ "R$ ".valorDbForm($veiculo['vl_ferramentas']) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
