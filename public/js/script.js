...
//função que controla para não ter mais de 4 anos na data
function controlaAno(ele){
    if(ele.value){
        arrayData = ele.value.split('-');
        controle = arrayData[0];
        if(controle.length > 3){
            controle = controle.substring(0,4);
            ele.value = controle + '-' + arrayData[1] + '-' + arrayData[2];
        }
    }
}
//fim função que controla para não ter mais de 4 anos na data
